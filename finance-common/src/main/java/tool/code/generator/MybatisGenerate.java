...
//				String className = GenerateUtils.toUpper(cn.substring(0));
				...
				String tplPath = "";
				if (db.equals("mysql")) {
					tplPath = StringUtils.replace(realProjectPath + "/finance-common/generate/template/mysql", "/", separator);
				} else if (db.equals("oracle")) {
					tplPath = StringUtils.replace(realProjectPath + "/finance-common/generate/template/oracle", "/", separator);
				}
...
//						new String[] { separator, separator });
				String dalPath = StringUtils.replaceEach(realProjectPath + "/finance-dal" + "/src/main/java/" + StringUtils.lowerCase(packageName) + "/dal", new String[] { "/", "." },new String[] { separator, separator });
				String bizPath = StringUtils.replaceEach(realProjectPath + "/finance-biz" + "/src/main/java/" + StringUtils.lowerCase(packageName) + "/biz", new String[] { "/", "." },new String[] { separator, separator });
				String webPath = StringUtils.replaceEach(realProjectPath + "/finance-admin" + "/src/main/java/" + StringUtils.lowerCase(packageName) + "/web", new String[] { "/", "." },new String[] { separator, separator });
				
				// mybatis文件生成地址
				
				String batisPath = StringUtils.replace(realProjectPath + "/finance-dal/src/main/resources/sqlmap", "/", separator);
